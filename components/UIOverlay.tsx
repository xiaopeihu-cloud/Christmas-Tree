...
export const UIOverlay: React.FC<UIOverlayProps> = ({ 
  ...
  const buttonClass = "w-10 h-10 rounded-full border border-white/20 bg-black/20 backdrop-blur-md flex items-center justify-center transition-all duration-500 hover:bg-white/10 hover:border-white/50 hover:scale-105 active:scale-95 group";

  // Silver Fairy Hand Style
  ...
  return (
    <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-6 z-10">
      ...
      {/* Top Bar: Music Control (Minimal Elegant) */}
      <div className="w-full flex justify-end items-start pointer-events-auto mt-2 mr-2">
        <button 
          onClick={toggleMusic}
      ...
      {/* Jingle Hand Button (Minimal Elegant) */}
      ...
      {isMagicHandOpen && (
        <div 
          ref={modalRef}
          className="absolute bottom-20 right-6 w-72 bg-black/80 backdrop-blur-xl border border-white/10 rounded-xl p-6 shadow-2xl pointer-events-auto origin-bottom-right animate-[fadeIn_0.3s_ease-out]"
        >
          <h3 className="text-sm font-luxury text-gray-200 mb-4 border-b border-white/10 pb-2 tracking-widest text-center">GESTURES</h3>
          
          <div className="space-y-4 text-xs text-gray-400 font-serif">
          ...
          <div className="mt-5 pt-3 border-t border-white/10">
            ...
            {simulationMode && (
              <div className="grid grid-cols-3 gap-2 mt-2">
                <button onClick={() => setManualGesture('Open_Palm')} className="py-1 bg-white/5 hover:bg-white/10 rounded text-[9px] text-center border border-white/10 text-gray-300">Open</button>
                <button onClick={() => setManualGesture('Closed_Fist')} className="py-1 bg-white/5 hover:bg-white/10 rounded text-[9px] text-center border border-white/10 text-gray-300">Close</button>
                <button onClick={() => setManualGesture('Pointing_Up')} className="py-1 bg-white/5 hover:bg-white/10 rounded text-[9px] text-center border border-white/10 text-gray-300">Point</button>
              </div>
            )}
          </div>
        </div>
